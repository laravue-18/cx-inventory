...
@section('title') Colours @endsection
...
@section('content')
    ...
    <div class="element-box">
        <div class="my-2 text-right">
            <a href="{{ route('user.colours.create') }}" class="btn btn-primary mr-3">New Colour</a>
        </div>
                ...
                <tbody>
                    @foreach($rows as $row)
                        <tr>
                            <td>{{ $row->name }}</td>
                            <td>
                                <a href="{{ route('user.colours.edit', $row->id) }}">
                                ...
                                <form
                                    ...
                                    action="{{ route('user.colours.destroy', $row->id) }}" method="POST"
                                    style="display: none;"
                                >
                                    @csrf
                                    @method('delete')
